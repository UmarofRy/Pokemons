...
function hammasi(movieArray) {
   ...
   movieArray.map((kino) => {
      div1.innerHTML += `
            <div class="movie">
                <p style="color: white; margin-left:214px; padding:8px 0px;  border-top-right-radius: 20px; width: 36px; height: 34px;  background: cornflowerblue;"  >${kino.num}</p>
                <h3>${kino.name}</h3>
                <img src="${kino.img}" alt="${kino.name}">
                <p class="type">${kino.type}</p>
                <p class="candy" >Candy count: ${kino.candy_count}</p>
                <p class="candiy">${kino.avg_spawns} kg</p>
                <p class="qizil">${kino.weaknesses}</p>
               <p class="soat" style="margin-bottom: 30px; width: 52px;  border-bottom-left-radius: 20px;  height: 34px; padding: 8px 8px 5px 2px; ">${kino.spawn_time}</p>
...
let qidir = () => {
   ...
   let newKino = pokemons.filter((movie) =>
...
let sortFunction = () => {
   ...
   switch (select) {
      ...
      case "name_za":
         ...
         break; 
      case "num_asc":
         sortedPokemons.sort((a, b) => a.num.localeCompare(b.num)); 
         break;
      case "num_desc":
         sortedPokemons.sort((a, b) => b.num.localeCompare(a.num)); 
         break;
      default:
         break;
   }

   hammasi(sortedPokemons); 
};
